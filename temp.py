...
def extract_content(sent_list,entity1,entity2):
    ...
    for i in range(startpoint,len(sent_list)):
        if num_appear < 3 and entity_pair[0] in sent_list[i][0] and entity_pair[1] in sent_list[i][0] : #當句
            case1 = i
            index.append(case1)            
        elif num_appear < 3 and i >= 2 and i <= len(sent_list)-3 and entity_pair[0] in sent_list[i-2:i+3][0] and entity_pair[1] in sent_list[i-2:i+3][0] : #前後兩句
            case2 = [i for i in range(i-2,i+3)]
            index.append[case2]
        else:
            case3 =  [i for i in range(index1-2,index1+3)]
            
            case3.extend(range(index2-2,index2+3))
            index = case3 #如果都沒有一起出現, 就抓各自第一次出現時句子的上下四句
            break
    return index
#___________________________________________________
...
def relation_list(sentence_list,DF):
    ...
    return content  #這邊處理得不好 一定要再處理好一點
#___________________________________________________
#透過訓練資料建立namelist 清單
def nameList(DF):
    name_list = []
